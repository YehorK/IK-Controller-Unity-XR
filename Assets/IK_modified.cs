...
//[ExecuteInEditMode]
public class IK_modified : MonoBehaviour
{
    public Transform ik;
    ...
    private List<Transform> jointStates = new List<Transform>();

    private Vector3 ik_position_update;

    public List<List<Quaternion>> recordedJointStates = new List<List<Quaternion>>();
    ...
    void Update()
    {
        ...
        //    if (isRecording)
        //    {
        //        recordedJointStates.Clear(); // Clear previous recording data
        ...
        ik_position_update = ik.position;
        ...
        if (isPlaying)
        {
            ...
            ReplayJointAngles(robot);
        }

        if (isRecording)
        {
            RecordJointAngles(robot);
        }
    ...
    public static void ApplyJointSolution(List<string> solutionStatus, double[,] jointSolutions, int solutionIndex, List<Transform> robotJoints)
    {
        ...
        if (solutionStatus[solutionIndex] != "NON DISPONIBLE")
        {
            ...
            for (int i = 0; i < robotJoints.Count; i++)
            {
                ...
                //Debug.Log(robotJoints[i].name + " --> " + robotJoints[i].localEulerAngles);
            }
        }
        else
        {
    ...
    public void RecordJointAngles(List<Transform> robotJoints)
    {

        Debug.Log("Inside of RecordJointAngles");
        // Capture the current rotation of each joint
        List<Quaternion> currentFrameRotations = new List<Quaternion>();

        //  string debugInfo = "Recording Frame: ";

        foreach (Transform joint in robotJoints)
        {
            //currentFrameRotations.Add(joint.localRotation);

            Quaternion jointRotation = joint.localRotation;
            currentFrameRotations.Add(jointRotation);

            // debugInfo += $"{joint.name}: {jointRotation.eulerAngles} | ";

        }
        recordedJointStates.Add(currentFrameRotations); // Add this frame's rotations
                                                        //  Debug.Log(debugInfo); // Output the recorded frame rotation
        Debug.Log("Count --> " + recordedJointStates.Count);
    }


    public void ReplayJointAngles(List<Transform> robotJoints)
    {
        Debug.Log("Inside of RecordJointAngles");
        Debug.Log("recordedJointStates count --> " + recordedJointStates.Count);
        if (playbackFrame < recordedJointStates.Count)
        {
            // Set each joint's rotation to the recorded value at this frame
            for (int i = 0; i < robotJoints.Count; i++)
            {
                robotJoints[i].localRotation = recordedJointStates[playbackFrame][i];
            }
            playbackFrame++; // Move to the next frame

            if (playbackFrame >= recordedJointStates.Count)
            {
                isPlaying = false; // Stop playback when frames are done
